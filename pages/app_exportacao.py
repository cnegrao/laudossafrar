...
@st.cache_data
def consultar_dados(tabela_laudo):
    """Executa a consulta e armazena em cache para evitar recomputação."""
    sql = f"SELECT * FROM {tabela_laudo}"
    return run_select(sql)


def gerar_excel(df):
    """Cria um arquivo Excel em memória e retorna os bytes."""
    output = io.BytesIO()
...
def main():
    ...
    if st.button("🔍 Carregar Dados"):
        try:
            ...
            df_laudo = consultar_dados(tabela_laudo)
